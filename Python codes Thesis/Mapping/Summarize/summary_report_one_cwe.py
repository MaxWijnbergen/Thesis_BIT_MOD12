...
# This code summarizes the found correlations in the text files. For every TTP it counts the amount of times it is correlated to every CWE  and it is the same for the CWE. For every CWE it counts the amount times it is correlated to every TTP. It also Counts the amount of every TTP in the file and the amount of every CWE in the file. It is the same code as summary_report_no_dups.py
...
# Main process
input_filepath = 'filtered_matched_cve_one_cwe_ttp.txt'
output_filepath = 'summary_report_one_cwe.txt'
