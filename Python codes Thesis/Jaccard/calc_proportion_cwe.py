...
# This code calculates a ratio that shows the percentage of a specific CWE being correlated to a specific TTP. It is calculated by dividing the intersection by the total of the CWE, so not by the union
...
def calculate_jaccard(ttp_counts, cwe_counts, ttp_cwe_mapping):
    ...
    for ttp, cwe_map in ttp_cwe_mapping.items():
        ...
        for cwe, intersection_count in cwe_map.items():
            ...
            jaccard_index = intersection_count / total_cwe if total_cwe != 0 else 0
            jaccard_indices[(ttp, cwe)] = (jaccard_index, total_cwe, ttp_counts[ttp])
...
output_filepath = 'proportion_indices_cwe.txt'
